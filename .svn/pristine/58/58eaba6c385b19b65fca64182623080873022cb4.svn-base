...
<c:set var="submenu" value="formsubmissions" />
	...
	<div class="content">
	  ...
	  <table width="100%" border="0" cellspacing="0" cellpadding="8" class="companiesTable">
	    <tr>
		  ...
		  <th>Email</th>
		  <th>Phone</th>
		  <th>${ company.name eq 'totalqueue2' ? 'Type' : company.name eq 'bluewarner' ? 'Promo Code' : 'Submitted From' }</th>
		  ...
		  <c:set var="count" value="0" />
				...
				<c:forEach items="${emailsList}" var="e">
						 	...
						 	<c:if test="${user.userType.value != 'Company Staff'}">		 	
						 		...
						 		<td>
						 			...
						 			<c:if test="${company.name eq 'metroquicash' }">
						 				| <a href="downloadSubmissionPDF.do?submissionId=${e.id}" target="_formpdf">PDF</a>
						 			</c:if>
						 		</td>
						 		<c:choose>
						 		...
						 		<c:when test="${e.formName=='Careers' and company.name eq 'nwdi'}">
						 		<td valign="middle">
						 			Attachment sent to ${company.email }
						 		</td>
						 		</c:when>  
										...
										hp.style.visibility = "Hidden";
							 			...
							 			<div>
							 				<div style="width: 10%; float: left;" >
													...
													--%>
												<c:choose>
													<c:when test="${company.name eq 'metroquicash' }">
														<c:forTokens items="${e.uploadFileName }" delims="#%#" var="fileName">
															<a href="${httpServer}/message_attachments/${fileName}" id="hoverover" style="cursor:pointer;" onMouseOver="ShowPopup(this,${e.id});" onMouseOut="HidePopup(${e.id});"><img src="images/Email_attach.png" align="middle"></a>
														</c:forTokens>
													</c:when>
													<c:otherwise>
