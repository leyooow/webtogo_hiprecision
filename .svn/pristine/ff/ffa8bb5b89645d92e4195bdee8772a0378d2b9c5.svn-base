...
<div class="contentWrapper" id="contentWrapper">
    ...
    <div class="mainContent">
	  ...
	  <div class="downloadsBox"> 
		...
		<form action="watsonmembers.do" method="post">
			<table width="100%">
				<tr>
					<td>Date From:</td>
					<td>
		...
		<table>
			<tr>
				<form action="downloadWatsonMemberDetails.do" method="post" name="downloadForm" >
					<%@include file="includes/memberDownloadSettings.jsp"  %>
					<c:forEach items="${list}" var="fieldName" >
						<input type="hidden" name="fieldName" value="${fieldName}">
					</c:forEach>	
					<td colspan="3">Download Members List in Excel Format</td>
					<td>
	  ...
	  <c:if test="${watsonWinner ne null }">
	  	<div class="downloadsBox">
	  		<table>
	  			...
	  			<tr>
	  				<td colspan="2">Contact Details : ${watsonWinner.mobile ne null ? watsonWinner.mobile : watsonWinner.telNo }</td>
	  			</tr>
	  		</table>
	  	</div>
	  </c:if>
	  ...
	  <div class="clear"></div>
 	  ...
 	  <table width="100%" border="0" cellspacing="0" cellpadding="8" class="companiesTable">
						...
						<c:forEach items="${watsonResults }" var="wr">
							<tr ${(count%2 == 1) ? 'class="oddRow"' : ''}>
								<c:set var="count" value="${count+1}" />
								<td>${wr.name }</td>
								<td>${wr.email }</td>
								<td>${(wr.telNo eq null) ? wr.mobile : wr.telNo }</td>
								<td><fmt:formatDate value="${wr.createdOn }" pattern="yyyy-MM-dd"/></td>
								<!-- <td></td> -->
	 	  					</tr>
 	  					</c:forEach>		
		</table>
