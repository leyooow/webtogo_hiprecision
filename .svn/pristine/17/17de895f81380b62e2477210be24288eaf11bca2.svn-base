...
<%-- 	<c:if test="${customFieldDownload == null}">	 --%>
    <div class="mainContent">
	  ...
	  <div class="downloadsBox"> 
		...
		<form action="bluewarnerraffle.do" method="post">
			<table width="100%">
				<tr>
					<td>Date From:</td>
					<td>
		...
		<table>
			<tr>
				<form action="downloadBluewarnerRaffleEntryDetails.do" method="post" name="downloadForm" >
 	  ...
 	  <table width="100%" border="0" cellspacing="0" cellpadding="8" class="companiesTable">
						...
						<c:forEach items="${emailsList}" var="wr">
							<tr ${(count%2 == 1) ? 'class="oddRow"' : ''}>
								...
								<!--<c:set var="emailcontent" value="${wr.email_content }"/>
								<c:set var="splitted" value="${fn:split(emailcontent,'<br/><br/>')}"/>
								 
									${splitted[0]}Name : elsbeth<br/><br/>
									${splitted[1]}Address : ortigas<br/><br/>
									${splitted[2]}Email : ealquintos@gmail.com<br/><br/>
									${splitted[3]}Cnumber : 09<br/><br/>
									${splitted[4]}Receipt Number : kgshrgrh<br/><br/>
									${splitted[5]}Promo Code : hwoghor<br/><br/>
								
								
								<c:set var="add" value="${splitted[1]}"/>
								<c:set var="rec" value="${splitted[4]}"/>
								<c:set var="pro" value="${splitted[5]}"/>
								<c:set var="address" value="${fn:split(add,'Address : ')}"/>
								<c:set var="receipt" value="${fn:split(rec,'Receipt Number : ')}"/>
								<c:set var="promo" value="${fn:split(pro,'Promo Code : ')}"/>
								  -->
