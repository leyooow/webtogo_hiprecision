<%@taglib uri="/webtogo-tags" prefix="webtogo" %>
...
<div class="logo-incentel">
  <c:if test="${not empty companyLogo}"> 
    <img style="padding:5px;border:1px solid #d9d8c8" src="images/${companyLogo}" />
  </c:if> 
</div>
...
<%--  <c:choose>
    <c:when test="${companyEmail ne null}">
      <p style="font:14pt arial"> Please contact support at ${companyEmail}</p>
    </c:when>
