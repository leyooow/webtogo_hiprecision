...
<c:if test="${pagingUtil.totalPages > 1}">
...
<c:if test="${pagingUtil.hasPrev}">
	<c:url var="pageLinkFinalPrev" value="${pageLinkSub}">
		<c:param name="page" value="${pagingUtil.currentPageNo - 1}"/>
	</c:url>
...
<c:forEach items="${pagingUtil.pages}" var="pageNum">
	<c:choose>
		<c:when test="${pageNum eq pagingUtil.currentPageNo}">
...
<c:if test="${pagingUtil.showLast}">
	 <c:url var="pageLinkFinalLast" value="${pageLinkSub}">
			<c:param name="page" value="${pagingUtil.totalPages}"/>
		</c:url>
