...
<c:set var="navMenu" value="doctors" />
							...
							<h1>${page.title}</h1>
					  		<div class="welcomeNote">
						        ...
						        <img src="images/grayOvalBr.jpg" class="ibr" />
							 	<table>
							 		<c:forEach items="${group.enabledCategories}" var="category" varStatus="counter">
							 			<th align="left" style="padding-bottom:5px;font-size:14px;">
							 				${category.name}
							 			</th>
							 			<tr>
							 				<c:forEach items="${category.items}" var="categoryItem" varStatus="counter">
							 					<td width="33%"><a href="${menu['doctors'].url}?item_id=${categoryItem.id}">${categoryItem.name} (${categoryItem.brand.name})</a></td>
