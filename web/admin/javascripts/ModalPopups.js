...
var ModalPopups = {
    ...
    _createAllLayers: function(id, title, message, parameters) {
        ...
        for(x in bt) {
            if(bt[x] == "ok")
                oPopupFooter.innerHTML += "<input name='" + id + "_okButton' id='" + id + "_okButton' type=button value='" + okButtonText + "' style='background:#126ba2; border:1px solid #005635; color:#fff; margin-left:5px; text-transform: capitalize; width: auto; overflow: visible;	padding:1px 8px' onclick='" + onOk + "'/>";
            if(bt[x] == "yes")
