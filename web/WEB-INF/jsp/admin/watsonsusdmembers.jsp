...
<div class="contentWrapper" id="contentWrapper">
    ...
    <div class="mainContent">
	  ...
	  <div class="downloadsBox"> 
		...
		<form action="watsonsusdmembers.do" method="post">
			<table width="100%">
				<tr>
					<td>Date From:</td>
					<td>
		...
		<table>
			<tr>
				<form action="downloadWatsonSUSDMemberDetails.do" method="post" name="downloadForm" >
	  ...
	  <c:if test="${watsonWinner ne null }">
	  	<div class="downloadsBox">
	  		<table>
	  			...
	  			<tr>
	  				<td colspan="2">Contact Details : ${watsonWinner.contactNumber }</td>
	  			</tr>
	  		</table>
	  	</div>
	  </c:if>
	  ...
	  <div class="clear"></div>
 	  ...
 	  <table width="100%" border="0" cellspacing="0" cellpadding="8" class="companiesTable">
						...
						<c:forEach items="${watsonSUSDResults}" var="wr">
							<tr ${(count%2 == 1) ? 'class="oddRow"' : ''}>
								<c:set var="count" value="${count+1}" />	
								<td>${wr.name }</td>
								<td>${wr.mobileNumber}</td>
								<td>${wr.contactNumber }</td>
								<td>${wr.email }</td>
								<td>${wr.birthday }</td>
								<td>${wr.points }</td>
								<!-- <td></td> -->
