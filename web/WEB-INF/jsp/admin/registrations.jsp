...
<c:set var="submenu" value="registrations" />
<c:set var="pagingAction" value="registrations.do" />
	...
	<div class="content">
	  ...
	  <div class="downloadsBox">
	    ...
	    <table border="0" cellspacing="0" cellpadding="5">
		  ...
		  <tr>
		  	<form action="registrations.do" method="get">
		  	<input type="hidden" id="eventName" name="eventName" />
		  	Filter By&nbsp; &nbsp; 
		  		<select onchange="document.getElementById('eventName').value=this.value;">
		  			<option value="">Please Select</option>
<%-- 		  			<option value="Yoga Pool Party" <c:if test="${param.eventName eq 'Yoga Pool Party'}">selected="selected"</c:if>>Yoga Pool Party</option> --%>
					<option value="Raw Dessert Workshop" <c:if test="${param.eventName eq 'Raw Dessert Workshop'}">selected="selected"</c:if>>Raw Dessert Workshop</option>
		  			<option value="LiFE at the Park" <c:if test="${param.eventName eq 'LiFE at the Park'}">selected="selected"</c:if>>LiFE at the Park</option>
		  		</select>
...
<!-- 	  </ul> -->
  ...
  <c:set var="pagingAction" value="registrations.do?eventName=${param.eventName}" />
...
<ul class="pagination" style="width: auto;clear: initial;">
	...
	<c:if test="${pagingUtil.totalItems > 0}">
	  ...
	  <table width="100%" border="0" cellspacing="0" cellpadding="8" class="companiesTable">
	    <tr>
		  ...
		  <th>Action</th>
		  <th>Attachments</th>
		  <c:if test="${company.id eq 264}">
		    <th>Approve</th>
		  </c:if>
		  <c:set var="count" value="0" />
		</tr>	
				<c:forEach items="${registrationList}" var="e">
							...
							<c:set var="count" value="${count+1}" />
						 	<c:if test="${user.userType.value != 'Company Staff'}">		 	
						 		...
						 		<td>
						 		<c:choose>
						 			<c:when test="${company.name eq 'bluewarner' }">
						 				${e.otherDetailMap['promo']}
						 			</c:when>
						 			<c:otherwise>
						 				${e.otherField1}
						 			</c:otherwise>
	...
	<!--									</table>-->
						 		...
						 		<td>
						 		  <c:if test="${company.id eq 264 and e.formName eq 'Testimonials' and empty e.emailValid}">
							 		<a onclick="return confirm('Do you really want to approve this testimonial?');" href="approvesubmission.do?submissionId=${e.id}">Approve</a>
						 		  </c:if>				 		
						 		</td>
						 	</tr>
...
<c:if test="${not empty registrationList && user.userType.value != 'Company Staff'}"> 
	  <ul class="selectionList">
