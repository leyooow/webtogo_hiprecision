...
public class KuysenSalesRegistrationAction
		extends AbstractBaseAction
		implements KuysenSalesTransactionAware
{
	...
	public KuysenSalesRegistrationAction()
	{
		super();
	}
			...
			else if(StringUtils.isNotEmpty(kaptcha))
			{
				...
				if(kaptcha.equalsIgnoreCase(kaptchaExpected))
				{
					...
					else if(StringUtil.areNotEmpty(username, email, password))
					{
						...
						member.setInfo2(saveSignatureImage());
	...
	public String saveSignatureImage() {
		...
		destinationPath += File.separator + company.getName() + "/members/signatures";
		
		System.out.println(destinationPath);
		
		File destFolder = new File(destinationPath.replaceAll(" ", "_"));
		destFolder.mkdirs();
		...
		String filename = insertPostfix(signatureFileName.replace(" ", "_"),String.valueOf(new Date().getTime()));

		File origFile = new File(destinationPath + File.separator + filename);

		try {
			FileUtils.copyFile(signature, origFile);
		} catch (IOException e) {
	...
	public String register_client() {
		...
		kuysenClientBean.setClientCompany(clientCompany);
