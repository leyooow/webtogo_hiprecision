...
public class CategoryItemOtherFieldMachineDAO
		extends AbstractBaseDAO<CategoryItemOtherFieldMachine>
{
	public CategoryItemOtherFieldMachineDAO() {
		super();
	} 
	
	public CategoryItemOtherFieldMachine findByCategoryItemOtherFieldMachine(Company company, CategoryItem categoryItem, OtherField otherField)
	{
		...
		List<CategoryItemOtherFieldMachine> temp = this.findAllByCriterion(null, null, null, null, junc).getList();
	...
	/*for category item*/
	public ObjectList<CategoryItemOtherFieldMachine> findDescItemsWithPaging(Company company,Brand brand, List<OtherField> otherFields,List<String> otherFieldValues, int resultPerPage, int pageNumber, Order...orders) {
		ObjectList<CategoryItemOtherFieldMachine> items = null;
	...
	public ObjectList<CategoryItemOtherFieldMachine> findAll(Company company, CategoryItem categoryItem){
		Junction junc = Restrictions.conjunction();
	...
	public ObjectList<CategoryItemOtherFieldMachine> findAllByOtherFieldId(Company company, OtherField otherField){
		Junction junc = Restrictions.conjunction();
		junc.add(Restrictions.eq("company", company));
	...
	public ObjectList<CategoryItemOtherFieldMachine> findByCategoryItem(Company company, CategoryItem item) {
		...
		return findAllByCriterion(null, null, joinType, null, junc);
	}
	
	public CategoryItemOtherFieldMachine findByOtherFieldKeyword(Company company, CategoryItem categoryItem, String otherFieldName, boolean isKeyword)
	{
	...
	public CategoryItemOtherFieldMachine findWithNewSession(Long id, boolean openNewSession) {	
		if(openNewSession){
			if(id == null)
				return null;
			
			CategoryItemOtherFieldMachine ret = null;
			Session session = null;
			
			try{
				session = HibernateUtil.getSessionFactory().openSession();
				ret = ClassHelper.cast(session.get(CategoryItemOtherFieldMachine.class, id));
			}catch(Exception ex){
				ex.printStackTrace();
			}finally{
				session.close();
