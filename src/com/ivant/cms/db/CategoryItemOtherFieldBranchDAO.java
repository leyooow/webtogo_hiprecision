...
public class CategoryItemOtherFieldBranchDAO
		extends AbstractBaseDAO<CategoryItemOtherFieldBranch>
{
	public CategoryItemOtherFieldBranchDAO() {
		super();
	} 
	
	public CategoryItemOtherFieldBranch findByCategoryItemOtherFieldBranch(Company company, CategoryItem categoryItem, OtherField otherField)
	{
		...
		List<CategoryItemOtherFieldBranch> temp = this.findAllByCriterion(null, null, null, null, junc).getList();
	...
	/*for category item*/
	public ObjectList<CategoryItemOtherFieldBranch> findDescItemsWithPaging(Company company,Brand brand, List<OtherField> otherFields,List<String> otherFieldValues, int resultPerPage, int pageNumber, Order...orders) {
		ObjectList<CategoryItemOtherFieldBranch> items = null;
	...
	public ObjectList<CategoryItemOtherFieldBranch> findAll(Company company, CategoryItem categoryItem){
		Junction junc = Restrictions.conjunction();
	...
	public ObjectList<CategoryItemOtherFieldBranch> findAllByOtherFieldId(Company company, OtherField otherField){
		Junction junc = Restrictions.conjunction();
		junc.add(Restrictions.eq("company", company));
	...
	public CategoryItemOtherFieldBranch findByOtherFieldKeyword(Company company, CategoryItem categoryItem, String otherFieldName, boolean isKeyword)
	{
	...
	public CategoryItemOtherFieldBranch findWithNewSession(Long id, boolean openNewSession) {	
		if(openNewSession){
			if(id == null)
				return null;
			
			CategoryItemOtherFieldBranch ret = null;
			Session session = null;
			
			try{
				session = HibernateUtil.getSessionFactory().openSession();
				ret = ClassHelper.cast(session.get(CategoryItemOtherFieldBranch.class, id));
			}catch(Exception ex){
				ex.printStackTrace();
			}finally{
				session.close();
