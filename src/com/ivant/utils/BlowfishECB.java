...
/**
 * Implementation of the Blowfish encryption algorithm in ECB mode. This is the
 * base class for advanced modes like CFB and CBC. It is <b>not</b> recommended
 * to use ECB for encryption purposes, since data characteristics outside of the
 * block size are not obfuscated.
 */
public class BlowfishECB extends Blowfish
{
    ...
    /**
     * Initializes or resets the current instance.
     * @param key The key material, up to MAXKEYLENGTH bytes.
     * @param ofs Where to start reading the key.
     * @param len The number of key bytes. The minimum is zero, however that
     * means to secret at all involved in the encryption - still can be used for
     * cheap obfuscation techniques.
     */
    public void initialize(byte[] key, int ofs, int len)
    {
        ...
        for (int i = 0; i < PBOX_ENTRIES; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                build = (build << 8) | (((int) key[ofs]) & 0x0ff);
    ...
    /**
     * The default constructor. You <b>must</b> call initialize() afterwards
     * to do the final setup. Practical for reusing instances.
     */
    public BlowfishECB()
    {
    }
    ...
    /** @see blowfishj.BlowfishECB#initialize(byte[], int, int) */
    ...
    /**
     * To clear data in the boxes before an instance is freed, so no derived
     * key material is left in memory. Depending on the JVM and the underlying
     * OS the material might not get completely erased, however it is still
     * recommended to at least attempt it.<p>Notice that after the cleanup the
     * instance is invalid and <b>must</b> be initialized again!</p>
     */
    public void cleanUp()
    {
    ...
    /**
     * Self test routine to make sure the implementation is compatible and the
     * code is intact. Memory corruption of the boxes, although very unlikely, 
     * for instance could lead to lost data.
     * @return True if the self test passed, false if one of the checks failed.
     * In latter case the application <b>must not</b> continue! 
     */
    public static boolean selfTest()
    {
        ...
        testbf2 = new BlowfishECB(TESTKEY2.getBytes(), 0, TESTKEY2.length());
    ...
    /**
     * To check for a weak key. Some key material might potentially cause a
     * weak cipher setup to be created. It is not recommended using the instance
     * then although proper decryption is guaranteed. Usually repeating the
     * initialization with different key material (e.g. choosing a different
     * salt value) should bring things back to normal. 
     * @return True if a weak key was detected or false if the key is fine.
     */
    ...
    // to avoid the Blowfish CBC/CFB classes to interfere.

    protected final int encryptPrv(
        byte[] inbuf,
        int inpos,
        byte[] outbuf,
        int outpos,
    ...
    /**
     * Encrypts a byte buffer to another buffer. Rest data beyond the last block
     * boundary will not be processed.
     * @param inbuf The buffer with the plain-text data.
     * @param inpos Where to start reading the plain-text data.
     * @param outbuf Buffer to write the cipher-text data to.
     * @param outpos Where to start writing the cipher-text data.
     * @param len The number of bytes to encrypt.
     * @return The number of bytes to encrypted. 
     */
    public int encrypt(
        byte[] inbuf,
        int inpos,
        ...
        int len)
    {
        return encryptPrv(inbuf, inpos, outbuf, outpos, len);
    }

    ///////////////////////////////////////////////////////////////////////////

    /**
     * Decrypts a byte buffer to another buffer. Rest data beyond the last block
     * boundary will not be processed.
     * @param inbuf Buffer to write the cipher-text data to.
     * @param inpos Where to start writing the cipher-text data.
     * @param outbuf The buffer with the plain-text data.
     * @param outpos Where to start reading the plain-text data.
     * @param len The number of bytes to encrypt.
     * @return The number of bytes to encrypted. 
     */
    public int decrypt(
